...
struct FramebufferOGL {
    ...
    GLuint depth_tex;    // GL_DEPTH_COMPONENT32F texture; 0 if not created
    GLuint velocity_tex; // RG16F motion-vector texture (COLOR_ATTACHMENT1); 0 if not created
};
...
class GfxRenderingAPIOGL final : public GfxRenderingAPI {
  public:
    ...
    GLuint GetDepthTextureId(int fbId);
    GLuint GetVelocityTextureId(int fbId);
    // Enable/disable MRT velocity buffer write. Must be called before game rendering.
    // When false (default), only COLOR_ATTACHMENT0 is active (safe for all shaders).
    void SetVelocityBufferEnabled(bool enabled) { mVelocityEnabled = enabled; }
  ...
  private:
    ...
    bool mVelocityEnabled = false; // set true by SSGI when it wants the velocity MRT
