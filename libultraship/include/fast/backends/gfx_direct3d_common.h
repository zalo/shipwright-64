...
struct FramebufferDX11 {
    ...
    // Motion-vector velocity buffer (RG16F, written as COLOR_ATTACHMENT1 during game render)
    Microsoft::WRL::ComPtr<ID3D11Texture2D> velocity_texture;
    Microsoft::WRL::ComPtr<ID3D11RenderTargetView> velocity_rtv;
...
class GfxRenderingAPIDX11 final : public GfxRenderingAPI {
  public:
    ...
    // Returns the SRV for the velocity (motion-vector) texture, or nullptr if unavailable.
    ID3D11ShaderResourceView* GetVelocitySrv(int fbId);
    // Returns the SRV for the depth buffer, or nullptr if unavailable.
    ID3D11ShaderResourceView* GetDepthSrv(int fbId);
