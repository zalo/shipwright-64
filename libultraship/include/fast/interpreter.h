...
struct LoadedVertex {
    ...
    struct RGBA color;
    uint8_t clip_rej;
    // Previous-frame clip-space position (x, y, w) for motion-vector output.
    // Equals current position when no previous-frame matrix data is available.
    float prev_x, prev_y, prev_w;
};
...
class Interpreter {
  public:
    ...
    void Run(Gfx* commands, const std::unordered_map<Mtx*, MtxF>& mtx_replacements,
             const std::unordered_map<Mtx*, MtxF>* prev_mtx_replacements = nullptr);
    void EndFrame();
    ...
    // Post-process hook for screen-space effects (e.g. SSGI).
    // Called after the game has rendered to mGameFb, before blitting to screen.
    // Callback may update outFrameBuffer to redirect the final display texture.
    using PostProcessCallback = std::function<void(int gameFb, GfxRenderingAPI* rapi,
                                                    uint32_t width, uint32_t height,
                                                    float proj[4][4], float view[4][4],
                                                    uintptr_t& outFrameBuffer)>;
    PostProcessCallback mPostProcessCallback;
    bool mForceRenderToFb = false; // force game to render to mGameFb (required for post-effects)

    // Optional previous-frame matrix replacements for per-vertex motion vectors.
    // When set, GfxSpVertex() computes prev_x/y/w using the previous MP matrix.
    const std::unordered_map<Mtx*, MtxF>* mPrevMtxReplacements = nullptr;

    // Parallel "previous frame" matrix state — mirrors mRsp but uses mPrevMtxReplacements.
    float mPrevMVStack[11][4][4]{};
    float mPrevPMatrix[4][4]{};
    float mPrevMPMatrix[4][4]{}; // = mPrevMVStack[top] × mPrevPMatrix
