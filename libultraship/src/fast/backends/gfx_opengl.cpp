...
ShaderProgram* GfxRenderingAPIOGL::CreateAndLoadNewShader(uint64_t shader_id0, uint32_t shader_id1) {
    ...
    // Always-present per-vertex previous-frame clip position for velocity buffer
    prg->attribLocations[cnt] = glGetAttribLocation(shader_program, "aVtxPrevPos");
...
int GfxRenderingAPIOGL::CreateFramebuffer() {
    ...
    mFrameBuffers[i].depth_tex    = 0;
    mFrameBuffers[i].velocity_tex = 0;

    return i;
}

GLuint GfxRenderingAPIOGL::GetDepthTextureId(int fb_id) {
    return mFrameBuffers[fb_id].depth_tex;
}

GLuint GfxRenderingAPIOGL::GetVelocityTextureId(int fb_id) {
    return mFrameBuffers[fb_id].velocity_tex;
}
...
void GfxRenderingAPIOGL::UpdateFramebufferParameters(int fb_id, uint32_t width, uint32_t height, uint32_t msaa_level,
    ...
    if (fb_id != 0) {
        ...
        if (has_depth_buffer &&
            ...
            if (can_extract_depth && msaa_level <= 1) {
                // Use GL_DEPTH24_STENCIL8 to match every other FBO's depth format so that
                // glBlitFramebuffer (used for game depth-buffer effects) succeeds.
                // The depth value is still readable as sampler2D .r in the SSGI shader.
                if (fb.depth_tex == 0) {
        ...
        if (can_extract_depth && msaa_level <= 1) {
            if (fb.velocity_tex == 0) {
                glGenTextures(1, &fb.velocity_tex);
            }
            glBindTexture(GL_TEXTURE_2D, fb.velocity_tex);
            ...
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
            glBindTexture(GL_TEXTURE_2D, 0);
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, fb.velocity_tex, 0);
        }

        if (!fb.has_depth_buffer && has_depth_buffer) {
            if (can_extract_depth && msaa_level <= 1 && fb.depth_tex != 0) {
                // Use DEPTH_STENCIL_ATTACHMENT since the texture is GL_DEPTH24_STENCIL8
                glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, fb.depth_tex, 0);
            } else {
...
void GfxRenderingAPIOGL::StartDrawToFramebuffer(int fb_id, float noise_scale) {
    ...
    // Only manage draw buffers on FBOs that have the velocity texture attached.
    // Never call glDrawBuffers on the default framebuffer (fb_id==0) — its
    // draw buffer state is implicitly correct and GL_BACK is not reliably
    // accepted by all WebGL2 implementations.
    if (fb.velocity_tex != 0) {
        if (mVelocityEnabled) {
            static const GLenum kTwoBuffers[] = { GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1 };
            glDrawBuffers(2, kTwoBuffers);
        } else {
            // SSGI disabled — reset to single draw buffer so shaders without
            // the velocity output don't produce an INVALID_OPERATION error.
            static const GLenum kOneBuffer[] = { GL_COLOR_ATTACHMENT0 };
            glDrawBuffers(1, kOneBuffer);
        }
    }
}
