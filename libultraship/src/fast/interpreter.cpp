...
Interpreter::Interpreter() {
    mRsp = new RSP();
    mRdp = new RDP();
    mBufVbo = new float[MAX_TRI_BUFFER * (40 * 3)]; // 40 floats/vertex max (was 32; +3 for aVtxPrevPos + headroom)
}
...
void Interpreter::GfxSpMatrix(uint8_t parameters, const int32_t* addr) {
    ...
    // ---- Previous-frame parallel matrix update ----
    // Look up the same matrix address in mPrevMtxReplacements; fall back to
    // the current (already-substituted) matrix when no prev data is available.
    float prev_matrix[4][4];
    if (mPrevMtxReplacements) {
        if (auto it = mPrevMtxReplacements->find((Mtx*)addr); it != mPrevMtxReplacements->end()) {
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++) {
                    float v = it->second.mf[i][j];
                    int as_int = (int)(v * 65536.0f);
                    prev_matrix[i][j] = as_int * (1.0f / 65536.0f);
                }
        } else {
            memcpy(prev_matrix, matrix, sizeof(matrix)); // no entry → same as current
        }
    } else {
        memcpy(prev_matrix, matrix, sizeof(matrix)); // no prev replacements → same as current
    }

    if (parameters & mtx_projection) {
        if (parameters & mtx_load) {
            memcpy(mRsp->P_matrix, matrix, sizeof(matrix));
            memcpy(mPrevPMatrix,   prev_matrix, sizeof(prev_matrix));
        } else {
            MatrixMul(mRsp->P_matrix, matrix,      mRsp->P_matrix);
            MatrixMul(mPrevPMatrix,   prev_matrix,  mPrevPMatrix);
        }
        ...
        if ((parameters & mtx_push) && mRsp->modelview_matrix_stack_size < 11) {
            ...
            memcpy(mPrevMVStack[mRsp->modelview_matrix_stack_size - 1],
                   mPrevMVStack[mRsp->modelview_matrix_stack_size - 2], sizeof(matrix));
        }
        if (parameters & mtx_load) {
            ...
            memcpy(mPrevMVStack[mRsp->modelview_matrix_stack_size - 1], prev_matrix, sizeof(prev_matrix));
        } else {
            MatrixMul(mRsp->modelview_matrix_stack[mRsp->modelview_matrix_stack_size - 1], matrix,
                      mRsp->modelview_matrix_stack[mRsp->modelview_matrix_stack_size - 1]);
            MatrixMul(mPrevMVStack[mRsp->modelview_matrix_stack_size - 1], prev_matrix,
                      mPrevMVStack[mRsp->modelview_matrix_stack_size - 1]);
        }
    ...
    MatrixMul(mPrevMPMatrix, mPrevMVStack[mRsp->modelview_matrix_stack_size - 1],       mPrevPMatrix);
}
        ...
        if (mRsp->modelview_matrix_stack_size > 0) {
            ...
            if (mRsp->modelview_matrix_stack_size > 0) {
                MatrixMul(mRsp->MP_matrix, mRsp->modelview_matrix_stack[mRsp->modelview_matrix_stack_size - 1],
                          mRsp->P_matrix);
                MatrixMul(mPrevMPMatrix,   mPrevMVStack[mRsp->modelview_matrix_stack_size - 1],
                          mPrevPMatrix);
            }
        }
    }
...
void Interpreter::GfxSpVertex(size_t n_vertices, size_t dest_index, const F3DVtx* vertices) {
    for (size_t i = 0; i < n_vertices; i++, dest_index++) {
        ...
        // Previous-frame clip position for motion-vector output
        d->prev_x = v->ob[0] * mPrevMPMatrix[0][0] + v->ob[1] * mPrevMPMatrix[1][0] +
                    v->ob[2] * mPrevMPMatrix[2][0] + mPrevMPMatrix[3][0];
        d->prev_y = v->ob[0] * mPrevMPMatrix[0][1] + v->ob[1] * mPrevMPMatrix[1][1] +
                    v->ob[2] * mPrevMPMatrix[2][1] + mPrevMPMatrix[3][1];
        d->prev_w = v->ob[0] * mPrevMPMatrix[0][3] + v->ob[1] * mPrevMPMatrix[1][3] +
                    v->ob[2] * mPrevMPMatrix[2][3] + mPrevMPMatrix[3][3];
...
void Interpreter::GfxSpTri1(uint8_t vtx1_idx, uint8_t vtx2_idx, uint8_t vtx3_idx, bool is_rect) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // aVtxPrevPos — always last, matching registration order in CreateAndLoadNewShader.
        mBufVbo[mBufVboLen++] = v_arr[i]->prev_x;
        mBufVbo[mBufVboLen++] = clip_parameters.invertY ? -v_arr[i]->prev_y : v_arr[i]->prev_y;
        mBufVbo[mBufVboLen++] = v_arr[i]->prev_w;
    }
...
void Interpreter::SpReset() {
    ...
    // Copy current matrix state into the prev stack as an identity baseline.
    // GfxSpMatrix() will overwrite these as the display list sets up matrices.
    memcpy(mPrevMVStack, mRsp->modelview_matrix_stack, sizeof(mPrevMVStack));
    memcpy(mPrevPMatrix, mRsp->P_matrix, sizeof(mPrevPMatrix));
    memcpy(mPrevMPMatrix, mRsp->MP_matrix, sizeof(mPrevMPMatrix));
...
void Interpreter::StartFrame() {
    ...
    if (!ViewportMatchesRendererResolution() || mMsaaLevel > 1 || mForceRenderToFb) {
        mRendersToFb = true;
...
void Interpreter::Run(Gfx* commands, const std::unordered_map<Mtx*, MtxF>& mtx_replacements,
                      const std::unordered_map<Mtx*, MtxF>* prev_mtx_replacements) {
    mPrevMtxReplacements = prev_mtx_replacements;
    SpReset();
    ...
    if (mRendersToFb) {
        ...
        if (mMsaaLevel > 1) {
            ...
            // Invoke post-process callback (e.g. SSGI) before blitting to screen
            if (mPostProcessCallback) {
                mPostProcessCallback(mGameFb, mRapi,
                                     mCurDimensions.width, mCurDimensions.height,
                                     mRsp->P_matrix,
                                     mRsp->modelview_matrix_stack[0],
                                     mGfxFrameBuffer);
            }
        }
    } else if (mFbActive) {
