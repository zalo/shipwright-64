...
out vec4 vOutColor;
// Motion-vector output: previous-frame UV, written to COLOR_ATTACHMENT1.
// Silently ignored if only one draw buffer is active.
layout(location = 1) out vec2 vVelocity;
@end
// Previous-frame clip position passed from vertex shader
@{attr} vec3 vPrevClipPos;
...
void main() {
    ...
    @if(core_opengl || opengles)
    // Write previous-frame UV to velocity buffer (MRT attachment 1).
    // prev_w is stored in vPrevClipPos.z; clamp to avoid div-by-zero.
    float prevW = abs(vPrevClipPos.z) > 0.0001 ? vPrevClipPos.z : 0.0001;
    vVelocity = vPrevClipPos.xy / prevW * 0.5 + 0.5;
    @end
}