...
struct PSInput {
    ...
    // Previous-frame clip position (x, y, w) for motion-vector velocity output.
    // y is already DX-flipped (invertY applied in VBO construction).
    float3 prevClipPos : PREVCLIPPOS;
    @{update_floats(3)}
@for(i in 0..2)
    @if(o_textures[i])
...
// Dual-output: color to SV_TARGET0, motion-vector (previous-frame UV) to SV_TARGET1
struct PSOutput {
...
PSOutput PSMain(PSInput input, float4 screenSpace : SV_Position) {
    ...
    @if(o_alpha)
        ...
        @end
        @if(srgb_mode)
            psout.color = fromLinear(texel);
        @else
            psout.color = texel;
        @end
    @else
        @if(srgb_mode)
            psout.color = fromLinear(float4(texel, 1.0));
        @else
            psout.color = float4(texel, 1.0);
        @end
    @end

    // Velocity output: previous-frame UV in DX11 texture space (0=top-left).
    // prevClipPos.xy are DX clip-space (y already flipped by VBO code),
    // prevClipPos.z stores the clip-space w.
    {
        float prevW = abs(input.prevClipPos.z) > 0.0001 ? input.prevClipPos.z : 0.0001;
        float2 dx_ndc = input.prevClipPos.xy / prevW;
        psout.velocity.x = dx_ndc.x * 0.5 + 0.5;
        psout.velocity.y = 1.0 - (dx_ndc.y * 0.5 + 0.5);  // DX NDC y-up → UV y-down
    }
